...
function Luxt1.CreateWindow(libName, logoId)
  ...
  local function fadeAll(root, to, duration, visibilityAfter)
	...
	if visibilityAfter ~= nil then
		task.delay(duration, function()
			root.Visible = visibilityAfter
		end)
	end
end
        ...
        if not ok then 
            if current.KeyCode.Name == oldKey then 
                if LuxtLib.Enabled == true then
                   ...
                   fadeOut:Play()
fadeOut2:Play()
fadeAll(shadow, 1, 0.1, false) -- 👈 делаем скрытие с Visible = false
fadeAll(MainFrame, 1, 0.1, false)
...
shadow.Visible = true -- 👈 обязательно!
MainFrame.Visible = true -- 👈 обязательно!
        ...
        function sectionHandling:Section(sectionText)
            ...
            function ItemHandling:Button(btnText, callback)
                ...
                function ItemHandling:Toggle(toggInfo, callback)
                    ...
                    toggleInfo.Size = UDim2.new(0, 162, 1, 0)
                    toggleInfo.ZIndex = 2

...
cmd.add({"newesp"}, {"newesp", "Enable ESP using Sense"}, function()
    local Sense = getgenv().Sense or loadstring(game:HttpGet('https://sirius.menu/sense'))()
    getgenv().Sense = Sense
    local enemySettings = Sense.teamSettings.enemy
    enemySettings.enabled = true
    enemySettings.box = true
    enemySettings.boxColor = { Color3.new(1, 1, 1), 1 }
    enemySettings.boxOutline = true
    enemySettings.boxOutlineColor = { Color3.new(0, 0, 0), 1 }
    enemySettings.boxFill = false
    enemySettings.healthBar = true
    enemySettings.healthyColor = Color3.new(0, 1, 0)
    enemySettings.dyingColor = Color3.new(1, 0, 0)
    enemySettings.healthBarOutline = true
    enemySettings.healthBarOutlineColor = { Color3.new(0, 0, 0), 0.5 }
    enemySettings.healthText = true
    enemySettings.healthTextColor = { Color3.new(1, 1, 1), 1 }
    enemySettings.healthTextOutline = true
    enemySettings.healthTextOutlineColor = Color3.new(0, 0, 0)
    enemySettings.name = true
    enemySettings.nameColor = { Color3.new(1, 1, 1), 1 }
    enemySettings.nameOutline = true
    enemySettings.nameOutlineColor = Color3.new(0, 0, 0)
    enemySettings.weapon = false
    if not Sense.EspInterface then
        Sense.EspInterface = {}
    end
    Sense.EspInterface.getWeapon = function(player)
        local character = player.Character
        if character then
...
cmd.add({"unnewesp"}, {"unnewesp", "Disable ESP (Sense)"}, function()
    if getgenv().Sense then
        pcall(function()
            getgenv().Sense.Unload()
        end)
    end
    if getgenv().SenseESPMenu then
        pcall(function()
            getgenv().SenseESPMenu:Destroy()
        end)
        getgenv().SenseESPMenu = nil
    end
    getgenv().SenseESPEnabled = false
end)
...
cmd.add({"unnoclip"}, {"unnoclip", "Enable your player's collision"}, function()
    NAlib.disconnect("noclip")
    local char = getChar()
    if char then
        for _, v in pairs(char:GetDescendants()) do
...
cmd.add({"grenade"}, {"grenade", "Throw yourself like a grenade and explode"}, function()
    local player = game.Players.LocalPlayer
    local mouse = player:GetMouse()
    local character = player.Character or player.CharacterAdded:Wait()
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    local rootPart = character:WaitForChild("HumanoidRootPart")
    local launchPower = 120
    local angle = math.rad(45)
    local secondLaunchPower = Vector3.new(0, 150, 0)

    DoNotif("Click anywhere to launch yourself as a grenade!", 3)
    ...
    connection = mouse.Button1Down:Connect(function()
        ...
        local launchDir = (flatDirection * math.cos(angle) + Vector3.new(0, math.sin(angle), 0)) * launchPower

        local bodyVelocity = Instance.new("BodyVelocity")
        bodyVelocity.Velocity = launchDir
        ...
        local bodyVelocityUp = Instance.new("BodyVelocity")
        bodyVelocityUp.Velocity = secondLaunchPower
        bodyVelocityUp.MaxForce = Vector3.new(0, 1e5, 0)
        bodyVelocityUp.Parent = rootPart

        task.delay(0.15, function()
            bodyVelocityUp:Destroy()
        end)
...
cmd.add({"fling2"}, {"fling2 <player|all>", "Fling the specified player or everyone with enhanced logic"}, function(username)
    ...
    local function getTargetChars(name)
        local chars = {}
        if name:lower() == "all" then
            ...
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr.Name:lower():sub(1, #name) == name:lower() or (plr.DisplayName and plr.DisplayName:lower():sub(1, #name) == name:lower()) then
    ...
    local function applyEnhancedFling(targetChar)
        if isFlinging then return end
        isFlinging = true
        local char = player.Character
        if not (char and targetChar) then isFlinging = false return end
        local hrp = char:FindFirstChild("HumanoidRootPart")
        local targetHRP = targetChar:FindFirstChild("HumanoidRootPart")
        if not (hrp and targetHRP) then isFlinging = false return end
        ...
        local startTime = tick()
        local maxDuration = 5
        local flyAwayDistance = 50
        local upDownAmplitude = 5
        local upDownSpeed = 15
        while tick() - startTime < maxDuration and isFlinging do
            RunService.Heartbeat:Wait()
            local currentPos = targetHRP.Position
            local distFromStart = (currentPos - startPos).Magnitude
            if distFromStart > flyAwayDistance then
                break
            end
            local elapsed = tick() - startTime
            local yOffset = math.sin(elapsed * upDownSpeed) * upDownAmplitude
            hrp.CFrame = targetHRP.CFrame * CFrame.new(0, 1 + yOffset, 0)
        end
        bav:Destroy()
    ...
    local targets = getTargetChars(username)
    for _, targetChar in ipairs(targets) do
        applyEnhancedFling(targetChar)
    end
end)
